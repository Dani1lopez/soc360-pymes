...
| R-13 | ``.env`` contained a stale ``REDIS_URL=`` line incompatible with PR1 #260. The line was replaced with the structured ``REDIS_HOST/PORT/DB/PASSWORD`` quartet so the test suite can boot ``Settings``. The original content lives in ``.env.local-backup`` (gitignored). Verified on 2026-09-18: that substitution is the ONLY difference between the two files (16 shared keys, all with identical values), and both are gitignored and were never tracked or committed. | NOT a slice risk — local env hygiene only. | **No restore required.** Restoring the backup would reintroduce the rejected ``REDIS_URL`` and break local boot; the earlier guidance to restore before the commit gate was wrong and is superseded by this row. |
...
- `.env` `SECRET_KEY` measured 57 chars (not 67, as an earlier note claimed) while `Settings` requires >=128 (PR3 #260) and rejects low-entropy keys (>50% repeated characters) with no development exemption; the file also carried the stale `REDIS_URL` line. Both were fixed locally on 2026-09-18 with temporary user authorization: the `REDIS_URL` line became the structured `REDIS_HOST/PORT/DB/PASSWORD` quartet and the key was rotated to a 128-char value (repetition ratio 0.055). `Settings` now boots from `.env` alone. Tests are unaffected: `tests/conftest.py` injects its own 128-char key and test URLs via `os.environ.setdefault`, and the production `.env` is never touched. Expect a local re-login, because rotating the key invalidates locally signed tokens.
