...
Convención de ruta (canónica para este slice): el router se monta con `APIRouter(prefix="/assets")` y los endpoints de colección se registran bajo el path `"/"`, la misma convención que `app/modules/users/router.py`. La ruta efectiva de colección es por tanto `/api/v1/assets/`, y una petición a `/api/v1/assets` (sin barra final) responde `307 Temporary Redirect` hacia la forma con barra antes de alcanzar el handler, sin ejecutar la comprobación RBAC (una request no autenticada a la forma sin barra devuelve `307`, no `401`). Un `307` preserva método y cuerpo, por lo que la redirección no altera el contrato de escritura. Los clientes DEBEN seguir la redirección o usar directamente la forma con barra; el contrato observable de este slice se satisface sobre la ruta con barra final.
