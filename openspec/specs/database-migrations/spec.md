...
Defines the chained Alembic migration revisions (R1 and R2) that ship the F2 persistence layer, including the staged schema creation, per-revision downgrade behavior, and the per-PR model registration contract used by autogenerate.
...
- GIVEN a database is at revision R1
